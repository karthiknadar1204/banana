...
function App() {
  ...
  useEffect(() => {
    if (!samples.length) {
      axios.get("http://localhost:8080/samples").then((response) => {
  ...
  async function handleSubmit(ev: FormEvent) {
    ...
    setLoadingMessage("");
    const videoPath = "http://localhost:8080/" + videoResponse.data;
        ...
        <div className="py-8 flex flex-col justify-center">
          ...
          {videoUrl && (
            <a
              href={videoUrl}
              download
              className="bg-emerald-500 text-white px-4 py-2 rounded-full uppercase mt-4 inline-block"
            >
              Download Video
            </a>
          )}
